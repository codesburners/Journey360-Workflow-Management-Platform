...
# Import geocoding logic from places.py
try:
    ...
    try:
        ...
        def get_coordinates(name): return None, None
...
@router.get("/geocode")
async def geocode_proxy(query: str = Query(..., min_length=2)):
    """Robust geocoding proxy that uses ORS + Nominatim fallback via places.py"""
    try:
...
def resolve_airport(city: str) -> str:
    """Convert city name to IATA airport code. Strict for practicality."""
    key = city.strip().lower()
    if key in GLOBAL_AIRPORTS:
        return GLOBAL_AIRPORTS[key]
    
    # Check if input is ALREADY a valid IATA code in our global list
    if len(city.strip()) == 3:
        code = city.strip().upper()
        if code in GLOBAL_AIRPORTS.values():
            return code
            
    return None # Return None if not a known airport city
...
async def get_train_schedules(
    ...
    distance: float = Query(None, description="Travel distance in km (optional)")
):
    """
    ...
    # 2. Fallback to curated database ONLY if live API key is missing (Demo mode)
    if not schedules and not RAPIDAPI_KEY:
        schedules = _generate_mock_train_schedules(src, dst, travel_date, distance)
        source_type = "curated"
        print(f"[Transport] Using curated database (Demo Mode): {len(schedules)} trains")
    elif not schedules:
        print(f"[Transport] No live train results found for {origin}->{destination}. Mock fallback suppressed for accuracy.")
...
def _generate_mock_train_schedules(src: dict, dst: dict, date: str, distance: float = None) -> list:
    """Generate realistic train schedules using curated real Indian train data."""
    random.seed(hash(f"{src['code']}{dst['code']}{date}"))

    # ... (skipping REAL_TRAINS dict for brevity in target) ...
    ...
    # Use real distance if provided, else fallback to range
    base_distance = distance if distance else (200 + abs(code_hash % 600))
    ...
    for i in range(num_trains):
        ...
        km = base_distance + random.randint(-50, 50)
        
        # Indian Rail distance-based fare refinement (approximate per km + base)
        fare = {
            "2S": int(max(60, km * 0.25 + 40)),
            "SL": int(max(150, km * 0.55 + 80)),
            "3A": int(max(500, km * 1.45 + 250)),
            "2A": int(max(900, km * 2.10 + 450)),
            "1A": int(max(1800, km * 4.20 + 850)),
            "CC": int(max(300, km * 1.10 + 150)),
            "EC": int(max(800, km * 2.40 + 350)),
        }
        ...
        schedules.append({
            ...
            "runningDays": days,
            "classes": random.sample(list(fare.keys()), k=random.randint(3, 5)),
            "status": status,
            "distance": f"{km} km",
            "fare": fare
        })
...
async def get_flight_schedules(
    ...
    Enforces >300km distance for practicality.
    """
    src_iata = resolve_airport(origin)
    dst_iata = resolve_airport(destination)
    
    # Practicality Gate: No flights for very short distances or unknown hubs
    if not src_iata or not dst_iata:
        return {"source": "none", "count": 0, "schedules": [], "message": "No functional airports found for this route"}
        
    if distance and distance < 300:
        return {"source": "none", "count": 0, "schedules": [], "message": "Flight not practical for such a short distance"}
    ...
    if AVIATIONSTACK_KEY:
        try:
            ...
            if live_data and len(live_data) > 0:
                ...
                print(f"[Transport] AviationStack returned no flights. Mock fallback suppressed for accuracy.")
                return {
                    "source": "live",
                    "api": "AviationStack",
                    "origin": src_iata,
            ...
            print(f"[Transport] AviationStack API error: {e}")

    # 2. Fallback to mock data ONLY if no key is found (Demo mode)
    if not AVIATIONSTACK_KEY:
        mock_data = _generate_mock_flight_schedules(src_iata, dst_iata, travel_date, origin, destination)
        return {
            "source": "demo",
            "api": "Mock Data (No API Key)",
            "origin": src_iata,
            "destination": dst_iata,
            "date": travel_date,
            "count": len(mock_data),
    ...
    return {
        "source": "live",
        "api": "Flight API (No routes found)",
        "origin": src_iata,
        "destination": dst_iata,
        "date": travel_date,
        "count": 0,
...
def _generate_mock_flight_schedules(src_iata: str, dst_iata: str, date: str, origin_city: str, dest_city: str) -> list:
    ...
    for i in range(num_flights):
        ...
        # Distance-based flight pricing (base + per-hour rate) refined
        fare_base = int(2800 + flight_hours * 1350 + random.randint(-200, 800))
        fare_base = max(2500, fare_base) # Min airfare
...
PERFECT_BUS_ROUTES = {
    ...
    "mumbai-goa": {"distance": "590 km", "duration": "14h 00m", "km": 590, "hours": 14.0},
    "goa-mumbai": {"distance": "590 km", "duration": "14h 00m", "km": 590, "hours": 14.0},
    "bangalore-mysore": {"distance": "145 km", "duration": "3h 30m", "km": 145, "hours": 3.5},
    "mysore-bangalore": {"distance": "145 km", "duration": "3h 30m", "km": 145, "hours": 3.5},
    "delhi-agra": {"distance": "235 km", "duration": "4h 15m", "km": 235, "hours": 4.25},
    "agra-delhi": {"distance": "235 km", "duration": "4h 15m", "km": 235, "hours": 4.25},
    "chennai-coimbatore": {"distance": "510 km", "duration": "9h 00m", "km": 510, "hours": 9.0},
    "coimbatore-chennai": {"distance": "510 km", "duration": "9h 00m", "km": 510, "hours": 9.0},
    "hyderabad-pune": {"distance": "560 km", "duration": "10h 30m", "km": 560, "hours": 10.5},
    "pune-hyderabad": {"distance": "560 km", "duration": "10h 30m", "km": 560, "hours": 10.5},
    "delhi-lucknow": {"distance": "530 km", "duration": "8h 30m", "km": 530, "hours": 8.5},
    "lucknow-delhi": {"distance": "530 km", "duration": "8h 30m", "km": 530, "hours": 8.5},
    "chennai-katpadi": {"distance": "130 km", "duration": "3h 30m", "km": 130, "hours": 3.5},
    "katpadi-chennai": {"distance": "130 km", "duration": "3h 30m", "km": 130, "hours": 3.5},
}
...
async def get_bus_schedules(
    ...
    if not RAPIDAPI_KEY:
        mock_data = _generate_mock_bus_schedules(origin, destination, travel_date, is_india, distance)
                ...
                for b in buses_raw[:15]:
                    ...
                    try:
                        schedules.append({
                            ...
                            "duration": b.get("duration", b.get("travelTime", "0h 00m")),
                            "distance": b.get("distance", ""),
                            "fare": b.get("fare", b.get("fares", b.get("price", b.get("totalFare", b.get("baseFare", 0))))),
                            "currency": "INR" if is_india else "USD",
                            "seatsAvailable": b.get("seatsAvailable", b.get("availableSeats", b.get("available", 0))),
                            "totalSeats": b.get("totalSeats", b.get("maxSeats", 40)),
                            "rating": b.get("rating", b.get("ratings", 0.0)),
...
def _generate_mock_bus_schedules(origin: str, destination: str, date: str, is_india: bool, distance: float = None) -> list:
    ...
    return _generate_random_bus_schedules(origin, destination, date, is_india, distance)
...
- Chennai to Katpadi: ~130 km, 3.5 hours
...
4. Use VERY ACCURATE fares in {currency} for this specific route and bus type (check current market prices like RedBus)
...
def _generate_random_bus_schedules(origin: str, destination: str, date: str, is_india: bool) -> list:
    ...
    if perfect_route:
        ...
        # Use real distance if provided, else fallback to range
        base_distance = distance if distance else (100 + (code_hash % 800))
    ...
    for i in range(num_buses):
        ...
        # Fare varies by bus type - Refined for Indian market (based on RedBus/AbhiBus avg)
        fare_mult = {
            "AC Sleeper": 2.4, 
            "AC Seater": 1.8, 
            "Non-AC Seater": 1.1,
            "Volvo Multi-Axle": 2.8, 
            "Semi-Sleeper": 1.4, 
            "Luxury Coach": 3.0,
            "Double Decker": 1.9, 
            "Standard": 1.0, 
        ...
        base_fare_rate = 2.2 if is_india else 0.8 
        booking_fee = 50 if is_india else 5
        fare = int(max(200, base_distance * base_fare_rate * fare_mult.get(bus_type, 1.0) + booking_fee))
...
async def search_all_transport(
    ...
    Returns combined results for easy comparison using parallel fetching.
    """
    travel_date = date or datetime.now().strftime("%Y-%m-%d")

    # Fetch all data concurrently with a global timeout
    async def safe_fetch(task, name):
        try:
            return await asyncio.wait_for(task, timeout=12.0)
        except asyncio.TimeoutError:
            print(f"[Transport] {name} search timed out after 12 seconds")
            return {"count": 0, "schedules": [], "source": "timeout", "api": "Timeout"}
        except Exception as e:
            print(f"[Transport] {name} search failed: {e}")
            return {"count": 0, "schedules": [], "source": "error", "api": "Error"}

    # Calculate actual distance using coordinates
    from .places import get_coordinates
    src_coords = get_coordinates(origin)
    dst_coords = get_coordinates(destination)
    
    actual_dist = 400.0 # Default fallback
    if src_coords and dst_coords:
        ...
        # Apply 1.2x factor for actual road/rail distance vs displacement
        actual_dist = actual_dist * 1.2
    
    print(f"[Transport] Practical Distance Calc: {origin} -> {destination} = ~{actual_dist:.1f} km")

    # Define tasks with distance enrichment
    tasks = [
        safe_fetch(get_train_schedules(origin=origin, destination=destination, date=travel_date, distance=actual_dist), "Train"),
        safe_fetch(get_flight_schedules(origin=origin, destination=destination, date=travel_date, distance=actual_dist), "Flight"),
        safe_fetch(get_bus_schedules(origin=origin, destination=destination, date=travel_date, distance=actual_dist), "Bus")
    ]

    # Execute in parallel
    results = await asyncio.gather(*tasks)
    trains, flights, buses = results
    ...
    return {
        ...
        "buses": buses,
        "summary": {
            "totalTrains": trains.get("count", 0),
            "totalFlights": flights.get("count", 0),
            "totalBuses": buses.get("count", 0),
            "trainSource": trains.get("source", "unknown"),
            "flightSource": flights.get("source", "unknown"),
            "busSource": buses.get("source", "unknown"),
        }
    }
