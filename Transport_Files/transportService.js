...
const TRANSPORT_CONFIG = {
    'driving-car': { farePerKm: 2.5, basefare: 30, speedKmh: 35, color: '#F97316', icon: 'Car', label: 'Drive' },
    'foot-walking': { farePerKm: 0, basefare: 0, speedKmh: 5, color: '#10B981', icon: 'Footprints', label: 'Walk' },
    'cycling-regular': { farePerKm: 0, basefare: 0, speedKmh: 15, color: '#3B82F6', icon: 'Bike', label: 'Cycle' },
    // Legacy keys for UI compatibility
    train: { farePerKm: 0.6, basefare: 5, speedKmh: 45, color: '#3B82F6', icon: 'Train', label: 'Train' },
    bus: { farePerKm: 0.8, basefare: 8, speedKmh: 22, color: '#F97316', icon: 'Bus', label: 'Bus' },
    airport: { farePerKm: 2.5, basefare: 100, speedKmh: 35, color: '#8B5CF6', icon: 'Plane', label: 'Airport' },
    walk: { farePerKm: 0, basefare: 0, speedKmh: 5, color: '#10B981', icon: 'Footprints', label: 'Walk' },
};
...
const STATIONS = {
    trains: [
        { id: 'CST', name: 'Chhatrapati Shivaji Terminus', lat: 18.9398, lng: 72.8355, type: 'train', lines: ['Central', 'Harbour'] },
        { id: 'DR', name: 'Dadar', lat: 19.0178, lng: 72.8437, type: 'train', lines: ['Central', 'Western'] },
        { id: 'AN', name: 'Andheri', lat: 19.1197, lng: 72.8464, type: 'train', lines: ['Western', 'Harbour'] },
        { id: 'BV', name: 'Borivali', lat: 19.2283, lng: 72.8567, type: 'train', lines: ['Western'] },
        { id: 'TH', name: 'Thane', lat: 19.1854, lng: 72.9752, type: 'train', lines: ['Central', 'Trans-Harbour'] },
        { id: 'KR', name: 'Kurla', lat: 19.0653, lng: 72.8793, type: 'train', lines: ['Central', 'Harbour'] },
        { id: 'VL', name: 'Vile Parle', lat: 19.0989, lng: 72.8437, type: 'train', lines: ['Western'] },
        { id: 'GR', name: 'Goregaon', lat: 19.1641, lng: 72.8493, type: 'train', lines: ['Western'] },
        { id: 'CC', name: 'Churchgate', lat: 18.9350, lng: 72.8274, type: 'train', lines: ['Western'] },
        { id: 'PL', name: 'Panvel', lat: 18.9933, lng: 73.1170, type: 'train', lines: ['Harbour', 'Trans-Harbour'] },
    ],
    buses: [
        { id: 'B1', name: 'CSMT Bus Depot', lat: 18.9405, lng: 72.8358, type: 'bus', routes: ['1', '3', '6', '21'] },
        { id: 'B2', name: 'Dadar Bus Stand (E)', lat: 19.0180, lng: 72.8450, type: 'bus', routes: ['6', '22', '83'] },
        { id: 'B3', name: 'Andheri Bus Stand', lat: 19.1190, lng: 72.8480, type: 'bus', routes: ['203', '211', '305'] },
        { id: 'B4', name: 'BKC Bus Stop', lat: 19.0655, lng: 72.8680, type: 'bus', routes: ['332', '354'] },
        { id: 'B5', name: 'Borivali Bus Depot', lat: 19.2290, lng: 72.8575, type: 'bus', routes: ['294', '460'] },
        { id: 'B6', name: 'Haji Ali', lat: 18.9827, lng: 72.8121, type: 'bus', routes: ['83', '132', '161'] },
        { id: 'B7', name: 'Bandra Station', lat: 19.0544, lng: 72.8402, type: 'bus', routes: ['211', '215', '354'] },
        { id: 'B8', name: 'Juhu Beach', lat: 19.0896, lng: 72.8267, type: 'bus', routes: ['211', '231'] },
    ],
    airports: [
        { id: 'BOM1', name: 'Mumbai Airport T1 (Domestic)', lat: 19.0937, lng: 72.8568, type: 'airport' },
        { id: 'BOM2', name: 'Mumbai Airport T2 (International)', lat: 19.0896, lng: 72.8656, type: 'airport' },
    ]
};
...
async function searchPlaces(query) {
    ...
    // If no API key, fall back to mock stations
    if (!ORS_API_KEY) {
    ...
    try {
        ...
        console.warn('ORS geocode failed, using fallback:', err.message);
        return searchStopsFallback(query);
    }
}
...
async function generateRoute(sourceLat, sourceLng, destLat, destLng, modes = ['train', 'bus', 'airport']) {

    // If no API key, use mock fallback
    if (!ORS_API_KEY) {
        return generateRouteFallback(sourceLat, sourceLng, destLat, destLng, modes);
    }

    // Determine which ORS profiles to query
    const profilesToQuery = new Set();
    profilesToQuery.add('foot-walking'); // Always include walking
    ...
    // If all API calls failed, fall back to mock
    if (routes.length === 0) {
        console.warn('All ORS route requests failed, using fallback');
        return generateRouteFallback(sourceLat, sourceLng, destLat, destLng, modes);
    }

    // Tag routes with recommendations
    if (routes.length > 0) {
...
function generateRouteFallback(sourceLat, sourceLng, destLat, destLng, modes = ['train', 'bus', 'airport']) {
    const routes = [];
    ...
    for (const mode of modes.filter(m => m !== 'walk')) {
        ...
        if (srcStop.stop.id === dstStop.stop.id) continue;

        const legs = [];
        if (srcStop.distance > 0.1) {
            legs.push(buildLeg({ id: 'src', name: 'Your Location', lat: sourceLat, lng: sourceLng }, srcStop.stop, 'walk', 0));
        }
        ...
        if (destDist > 0.1) {
            legs.push(buildLeg(dstStop.stop, { id: 'dst', name: 'Destination', lat: destLat, lng: destLng }, 'walk', 2));
        }
    ...
    if (modes.includes('train') && modes.includes('bus')) {
        ...
        const dstFinal = { id: 'dst', name: 'Destination', lat: destLat, lng: destLng };

        if (srcTrain.stop.id !== midBus.stop.id) {
            const legs = [];
            if (srcTrain.distance > 0.1) {
                legs.push(buildLeg({ id: 'src', name: 'Your Location', lat: sourceLat, lng: sourceLng }, srcTrain.stop, 'walk', 0));
            }
