...
const TransportSearchPanel = ({ onSearch, onModeChange, activeModes, onLocationReady }) => {
    ...
    // Note: Destination starts blank — user must type their own origin/destination
        ...
        <div className="space-y-5">
            ...
            <div className="relative">
                <div className="space-y-3">
                    ...
                    <div ref={sourceRef} className="relative">
                        <div className="flex items-center gap-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-3 focus-within:border-emerald-500 dark:focus-within:border-emerald-500 transition-colors">
                            ...
                            <input
                                ...
                                placeholder="From — enter origin city or station"
                            ...
                            {source && !loadingSource && (
                                <button onClick={handleClearSource} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                                    <X size={14} />
                                </button>
                            )}
                        </div>
                    ...
                    <div className="flex justify-center -my-1">
                        <button
                            onClick={handleSwap}
                            className="p-2 bg-gray-100 dark:bg-slate-700 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 text-gray-400 hover:text-emerald-600 dark:hover:text-emerald-400 rounded-full transition-all duration-200 hover:scale-110"
                        >
                            <ArrowDownUp size={16} />
                        </button>
                    </div>
                    ...
                    <div ref={destRef} className="relative">
                        <div className="flex items-center gap-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-3 focus-within:border-red-500 dark:focus-within:border-red-500 transition-colors">
                            ...
                            <input
                                ...
                                placeholder="To — enter destination city or station"
                                className="flex-1 bg-transparent border-none outline-none text-sm text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500"
                            />
                            {loadingDest && <Loader2 size={14} className="text-red-500 animate-spin" />}
                            {destination && !loadingDest && (
                                <button onClick={handleClearDest} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                                    <X size={14} />
                                </button>
                            )}
                        </div>
            ...
            <div className="flex items-center gap-2">
                <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${scheduleMode === 'now'
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${scheduleMode === 'later'
                        ? 'bg-emerald-600 text-white shadow-md'
                        : 'bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-slate-700'
                        }`}
                >
                ...
                {scheduleMode === 'later' && (
                    <input
                        ...
                        className="ml-2 px-3 py-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg text-sm text-gray-700 dark:text-gray-300 outline-none focus:border-emerald-500"
                    />
                )}
            </div>
        </div>
