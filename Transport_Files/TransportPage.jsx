...
const TransportPage = () => {
    ...
    const handleSearch = useCallback(async (src, dst) => {
        ...
        try {
            const result = await transportService.generateRoute(src.lat, src.lng, dst.lat, dst.lng, activeModes);
            setRoutes(result);
    ...
    const handleModeChange = useCallback((modeId) => {
        setActiveModes(prev => {
            ...
            const next = isActive ? prev.filter(m => m !== modeId) : [...prev, modeId];
            // Re-search if we have source and dest
            if (sourceStop && destStop) {
                (async () => {
                    setIsSearching(true);
                    try {
                        const result = await transportService.generateRoute(sourceStop.lat, sourceStop.lng, destStop.lat, destStop.lng, next);
                        setRoutes(result);
